...
class ConcatTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $count = (new \ReflectionMethod('PhpFp\\Maybe\\Constructor\\Just::concat'))->getNumberOfParameters();
        $this->assertEquals($count, 1, 'Concat takes one parameter.');
    }
    public function testConcat()
    {
        $maybes = [Maybe::empty(), Maybe::of(2), Maybe::empty()];
        $acc = array_reduce($maybes, function ($acc, $x) {
            return $acc->concat($x);
        }, Maybe::empty());
